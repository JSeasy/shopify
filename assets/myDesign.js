class MyDesign {
    ...
    init() {
        ...
        this.getOverLayerImg().then(res => {
            if (res.data.design_mode === 1) {
                ...
                this.createCanvasFromJson(res.data.canvas_config, 0)
                $.this.bindDesignEvent($.this.canvas)
            } else if (res.data.design_mode === 2) {
                $.this.appendFixedModeElment(res.data)
                this.createCanvasFromJson(res.data.canvas_config, 0)
            } else {
                $.this.appendMutiModeElement(res.data)
    ...
    getOverLayerImg() {
        return new Promise((res) => {
            $.ajax({
                ...
                dataType: 'json',
                success(data) {
                    $.this.overLayUrl = JSON.parse(data.data.canvas_config)[0].overlayImage.src
    ...
    createCanvasFromJson(data, index) {
        $.this.canvas.loadFromJSON(JSON.parse(data)[index])
    }
    createCanvas() {
    ...
    appendFixedModeElment(data) {
        ...
        // 生成固定模式操作栏
        this.buildButtonFromFabricObject(JSON.parse(data.canvas_config)[0])
    }
    appendMutiModeElement(data) {
        const designModal = this.buildDesignModal()
        const previewModal = this.buildPreviewModal()
        $('body').append(designModal).append(previewModal)
        this.designModal = designModal
        this.previewModal = previewModal
        this.previewModalHiddenEvent()
        this.createStaticCanvas()
        this.buildMultiSelect(JSON.parse(data.canvas_config))
        // 生成固定模式操作栏
    }
    appendEntryBut(data) {
        const _this = this
    ...
    addToCardPost(hash, img) {
        ...
        dataItem.utf8 = '✓';
        dataItem.variant_id = $($.this.variantID).val();
        dataItem.id = $($.this.variantID).val();
    ...
    previewModalHiddenEvent() {
        this.previewModal.on(
            'hidden.bs.modal', () => {
                $('body').addClass('modal-open')
            }
        )
        ...
        return $(`<div class="myModal fade" id="myCropperModal" tabindex="-2" role="dialog" aria-labelledby="cropperModalLabel" aria-hidden="true" data-backdrop="false">
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-body cropperModalBody" >
              <div id="cropperTip">There are not enough pixels. may affect printing quality. Please enlarge the crop box or shrink the picture~</div>
                <div style="height:100%" id="cropperWrap"></div>
              </div>
              <div class="modal-footer">
    ...
    buildPreviewModal() {
        return $(`<div class="myModal fade" id="myPreviewModal" tabindex="-2" role="dialog" aria-labelledby="cropperModalLabel" aria-hidden="true" data-backdrop="false">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            ...
            <div class="modal-body previewModalBody">
    ...
    buildCropperArea(item, base64) {
        var $image = $(`<img id="image" src="${base64}" style="height:100%"/>`);
        $image.cropper({
    ...
    // 多选项模式
    buildMultiSelect(data) {
        const select = $(`
        <select id="selectNum" class="form-control">
        <option value=-1 id="selectTip"> <span > Select Number </span></option>
        </select>`)
        select.change((e) => {
            $(`#selectTip`).remove()
            $.this.createCanvasFromJson(JSON.stringify(data), select.val())
            this.createInput(data[select.val()])
        })
        data.map((item, index) => {
            select.append($(`<option value=${index}> <span > ${index + 1} </span></option>`))
        })
        $(`#multiModeWrap`).append(select)
    }
    ...
    createInput(data) {
        ...
        data.objects.map((item, index) => {
            const imgSelect = $(`
            <div>
                <h5 style="padding-left:10px;padding-bottom:0px;margin-bottom:0px;padding-top:10px">Choose ${index + 1}st cat </h5>
                    <div id="imgWrap">
                        <img src="https://img.kaikeba.com/13926121500202gjmb.png" />
                        <img src="https://img.kaikeba.com/13926121500202gjmb.png" />    
                        <img src="https://img.kaikeba.com/13926121500202gjmb.png" />
                        <img src="https://img.kaikeba.com/13926121500202gjmb.png" />    
                        <img src="https://img.kaikeba.com/13926121500202gjmb.png" />
            ...
            imgSelect.click((e) => {
                ...
                // console.log(e)
                const t = $.this.canvas.getObjects()[index].getObjects().find(item => item.type === 'image')
                const { height, width, scaleX, scaleY } = t
                // 获取真实高宽
                const realHeight = height * scaleY
                const realWidth = width * scaleX
                ...
                // t.set('src', e.target.src)
                // $.this.canvas.renderAll()

                t.setSrc(e.target.src, (img) => {
                    img.set({
                        scaleY: realHeight / img.height,
                        scaleX: realWidth / img.width,
                    })
            ...
            const wordInput = $(`
            <div>
            <h5 style="padding-left:10px;padding-bottom:0px;margin-bottom:0px;padding-top:10px">${index + 1}st cat name (0|10)  </h5>

            <input type="text"  class="form-control multiWordInput"/>
            </div>
            
            `)
            wordInput.on('input', (e) => {
                const t = $.this.canvas.getObjects()[index].getObjects().find(item => item.type === 'i-text')
                t.set('text', e.target.value)
                $.this.canvas.renderAll()
            })
            inputWrap.append(imgSelect).append(wordInput)
        })
        const previewButton = $(` <button type="button" class="preview btn btn-primary" style='margin-top:10px;width:100%;'>PRIVEIW</button>`)
        inputWrap.append(previewButton)
        previewButton.click(() => {
            // console.log($.this.canvas.toDataURL())
            $(`.previewModalBody`).empty()
            const img = new Image()

            img.src = $.this.canvas.toDataURL({ format: 'png', multiplier: (1 / $.this.zoom) * 2 })
            img.onload = () => {
                $(`.previewModalBody`).append(img)
                $.this.previewModal.modal(`toggle`)
            }
